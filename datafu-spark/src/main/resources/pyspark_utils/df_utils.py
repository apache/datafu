...
def dedup_random_n(df, group_col, max_size):
    """
    ...
    :param group_col: column to group by the records
    :param max_size: The maximal number of rows per group
    :return: DataFrame representing the data after the representation
    """
    jdf = _get_utils(df).dedupRandomN(df._jdf, group_col._jc, max_size)
    return DataFrame(jdf, df.sql_ctx)


def dedup_by_all_except(df, ignored_column):
    """
    Method for reducing rows when there is one column whose value is not important, but you don't want to lose any
    actual data from the other rows. For example if a server creates events with an autogenerated event id, and sometimes
    events are duplicated. You don't want double rows just for the event ids, but if any of the other fields are distinct
    you want to keep the rows (with their event ids)
    :param df: DataFrame
    :param ignored_column: The one column whose value you need only one of
    :param agg_function: Default is max
    :return:
    """
    jdf = _get_utils(df).dedupByAllExcept(df._jdf, ignored_column)
    return DataFrame(jdf, df.sql_ctx)
...
def activate():
    ...
    pyspark.sql.DataFrame.dedup_random_n = dedup_random_n
    pyspark.sql.DataFrame.dedup_by_all_except = dedup_by_all_except
